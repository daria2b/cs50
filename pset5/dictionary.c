...
bool check(const char* word)
{
    ...
    while(word[i] != '\0')
    {
        ...
        if(current->letters[tolower(word[i]) - 'a'] != NULL)
        {
            current = current->letters[tolower(word[i]) - 'a'];
            i++;
        }
...
bool load(const char* dictionary)
{
    ...
    while((ch = fgetc(inptr)) != EOF) 
    {
        ...
        if (ch != '\n')
        {
            ...
            {    
                current->letters[i] = malloc(sizeof(trie));
...
unsigned int size(void)
{
    ...
    // new function to go thru the trie
    count(head);
...
unsigned int count(trie* trie)
{
    //check if current trie is a word and add 1 to our counter, if it is
    if (trie->is_word == true)
        words++;
    
    for (int i = 0; i < 27; i++)
    {
        //recursion to go thru all branches of the trie
        if (trie->letters[i] != NULL)
            count(trie->letters[i]);
    }
