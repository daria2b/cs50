...
char* indexes(const char* path)
{
    //define some variables that will be used later to define if the file exists in directory
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    *length = size;
    ...
    // Create a buffer to store the file's contents, initially for 512 bytes
    static char* buffer;
    buffer = (char*)calloc(1, sizeof(BYTE) * size);
    
    // read contents of the file to the buffer
...
const char* lookup(const char* path)
{
    // USEFUL FUNCTIONS as of PSET6 specification:
    // int strcasecmp(const char *s1, const char *s2); to compare 2 strings.  It returns an integer less than, equal to, or greater than zero if s1 is found, respectively, to be less than, to match, or be greater than s2.
    // char *strcpy(char *dest, const char *src); The  strcpy()  function  copies the string pointed to by src, including the terminating null byte ('\0'), to the buffer pointed to by dest.
    ...
    //Find the last occurrence of a dot character, which will indicate where the file extention starts, returns a pointer
    ...
    //Create a variable to store output, 17 is the max number of characters in an input including \0. Declaring static char to keep the variable contents after function finishes
    static char type[17]; 
    memset(type, '\0', 17);
    
    //Go through all cases of possible file types and populate type string with correct response
    if (strncasecmp(".css", needle, 4) == 0)
        strcpy(type, "text/css");
    else if (strncasecmp(".html", needle, 5) == 0)
        strcpy(type, "text/html");
    else if (strncasecmp(".gif", needle, 4) == 0)
        strcpy(type, "image/gif");
    else if (strncasecmp(".ico", needle, 4) == 0)
        strcpy(type, "image/x-icon");
    else if (strncasecmp(".jpg", needle, 4) == 0)
        strcpy(type, "image/jpeg");
    else if (strncasecmp(".js", needle, 3) == 0)
        strcpy(type, "text/javascript");
    else if (strncasecmp(".php", needle, 4) == 0)
        strcpy(type, "text/x-php");
    else if (strncasecmp(".png", needle, 4) == 0)
...
{
    /* line should be of type "method SP request-target SP HTTP-version CRLF" */
    // USEFUL FUNCTIONS as of PSET6 specification:
    ...
    // int strncmp(const char *s1, const char *s2, size_t n); The strncmp() function is similar, except it compares the only first (at most) n bytes of s1 and s2.
    // char *strncpy(char *dest, const char *src, size_t n); The  strncpy() function is similar, except that at most n bytes of src are copied.
    // char *strstr(const char *haystack, const char *needle); The  strstr() function finds the first occurrence of the substring needle in the string haystack.
    
    /*We need to change the contents of abs_path and query, meaning need to access contents of the memory location with *, e.g. *input = toupper(*input) */
    
    /* Ensuring that the line structure is consistent with the rule "method SP request-target SP HTTP-version CRLF" */
    //Find the first occurrence of a space character. Q: do we need to allocate memory here for the char*?
